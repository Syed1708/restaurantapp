...
const { signAccessToken, signRefreshToken, REFRESH_EXPIRES, ACCESS_EXPIRES } = require('../utils/token');
...
async function register(req, res, next) {
  try {
    ...
    const user = await User.create({ name, email, passwordHash, role: role || 'waiter' });
...
async function login(req, res, next) {
  try {
    ...
    res.cookie(process.env.REFRESH_TOKEN_COOKIE_NAME || 'refreshToken', refreshTokenDoc.token, {
      ...cookieOptions,
...
async function refreshToken(req, res, next) {
  try {
    const tokenFromCookie = req.cookies[process.env.REFRESH_TOKEN_COOKIE_NAME || 'refreshToken'];
    const ip = req.ip || req.connection?.remoteAddress || '';

    if (!tokenFromCookie) return res.status(401).json({ message: 'Refresh token missing' });
    ...
    const accessToken = signAccessToken({ sub: String(stored.user._id), role: stored.user.role });

    // set new cookie (rotate)
    const cookieOptions = refreshTokenCookieOptions();
    res.cookie(process.env.REFRESH_TOKEN_COOKIE_NAME || 'refreshToken', newRt.token, {
      ...cookieOptions,
